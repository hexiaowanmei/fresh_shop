...
def add_cart(request):
    if request.method == 'POST':
        # 接收商品数量和id
        # 组装存储商品格式:[goods_id, num, is_select]
        ...
        if session_goods:
            ...
            # 第一次添加购物车，需组装购物车中的商品格式[[goods_id, num, is_select]]
...
def cart_price(request):
    if request.method == 'GET':
        ...
        return JsonResponse({'code': 200, 'msg': '请求成功', 'all_price': all_price,
                             'count': count, 'is_select_num': is_select_num,
                             'subtotal': subtotal})
...
def change_cart(request):
    if request.method == 'POST':
        # 修改商品的数量和选择状态
        # 其实就是修改session中商品信息，结构为[goods_id,is_select]
        # 1.获取商品id值和（数量或选择状态）
        goods_id = int(request.POST.get('goods_id'))
...
def del_cart(request, id):
    if request.method == 'POST':
        # 通过传入的商品id值，去session中查找，查找到要删除的数据
        session_goods = request.session.get('goods')
        for se_goods in session_goods:
            if se_goods[0] == id:
